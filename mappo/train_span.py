...
class SpanMAPPOTrainer:
    ...
    def compute_terminal_reward(self, answer: str, response_text: str) -> float:
        ...
        return 1.0 if response_text.strip() == str(answer).strip() else 0.0
    ...
    def train_step(self, questions: List[str], answers: List[str]) -> float:
        ...
        rewards = torch.stack(reward_traj)
        values = torch.stack(value_traj)
