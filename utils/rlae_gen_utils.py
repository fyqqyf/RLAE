...
def generate_ensemnble_response(
    ...
    **kwargs,
    ...
    while True:
        if weight_update_queue is not None and not weight_update_queue.empty():
            new_weights = weight_update_queue.get()
            if isinstance(new_weights, dict):
                new_weights = new_weights.get("weights", [])
            if len(new_weights) != len(model_actors_list):
                raise ValueError(
                    f"Expected {len(model_actors_list)} weights, got {len(new_weights)}."
                )
            weight_update_refs = [
