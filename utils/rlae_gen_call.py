...
from .unified_ray_actor import (
    get_remote_model_generator_class as get_unified_model_generator_class,
)
...
def setup_model_actors_and_data(
    ...
    actor_factory = (
        get_unified_model_generator_class
        if (use_unified_actor or span_mode)
        else get_remote_model_generator_class
    )
    model_actors_list = []
    for i, model_config in enumerate(config):
        ...
        if use_unified_actor or span_mode:
            actor_kwargs.update({"span_mode": span_mode, "span_length": span_length})
        model_actors_list.append(
            actor_factory(model_config["num_gpus"]).remote(**actor_kwargs)
        )
