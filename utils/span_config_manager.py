...
class SpanConfigValidator:
    ...
    def _validate_training_config(self, training_config: Dict[str, Any]):
        ...
        # Support two styles:
        # 1) flat: {algorithm, hyperparameters}
        # 2) nested: {ppo_span: {...}, mappo_span: {...}} or {ppo: {...}, mappo: {...}}
        valid_algorithms = {"ppo", "mappo"}

        def validate_hyperparams(hyperparams: Dict[str, Any]):
            if "learning_rate" in hyperparams:
                ...
                if isinstance(lr, str):
                    try:
                        ...
                        raise ValueError(f"Learning rate must be numeric, got {lr}") from exc
                if lr <= 0 or lr > 1:
            ...
            if "batch_size" in hyperparams:
                ...
                if isinstance(batch_size, str):
                    try:
                        ...
                        raise ValueError(f"Batch size must be an integer, got {batch_size}") from exc
                if batch_size <= 0:
                    raise ValueError(f"Batch size must be positive, got {batch_size}")

            if "span_length" in hyperparams:
                span_length = hyperparams["span_length"]
                if isinstance(span_length, str):
                    try:
                        span_length = int(span_length)
                    except ValueError as exc:
                        raise ValueError(f"Span length must be an integer, got {span_length}") from exc
                if span_length <= 0 or span_length > 128:
                    raise ValueError(
                        f"Span length must be between 1 and 128, got {span_length}"
                    )
        ...
        if "algorithm" in training_config:
            ...
            if "hyperparameters" in training_config:
                validate_hyperparams(training_config["hyperparameters"])
            return

        found_algo_like_block = False
        for key, value in training_config.items():
            if not isinstance(value, dict):
                continue
            algo_name = value.get("algorithm")
            if algo_name in valid_algorithms or any(x in key.lower() for x in valid_algorithms):
                found_algo_like_block = True
                if "hyperparameters" in value and isinstance(value["hyperparameters"], dict):
                    validate_hyperparams(value["hyperparameters"])
                elif "default_params" in value and isinstance(value["default_params"], dict):
                    validate_hyperparams(value["default_params"])

        if not found_algo_like_block:
...
class SpanConfigManager:
    ...
    def load_config(self, config_name: str) -> Dict[str, Any]:
        ...
        if raw_path.exists():
            ...
            if raw_path.suffix:
                config_path = self.config_dir / raw_path.name
            else:
